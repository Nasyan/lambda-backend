...
class TestCoreCrmDynamicCost:
    @pytest.mark.asyncio
    async def test_order_dynamic_cost_and_client_upsert(
        self, test_client, setup_crm_dynamic_cost_and_trigger
    ):
        """
        Проверяем автоматический расчет стоимости и логируем промежуточные состояния
        для отладки движка AST-формул.

        Этот тест не прошел
        """
        env = setup_crm_dynamic_cost_and_trigger
        instance_uuid = env["instance_uuid"]
        headers = env["headers"]
        clients_id = env["clients_template_uuid"]
        products_id = env["products_template_uuid"]
        ...
        print("\n" + "=" * 60)
        print("[DEBUG START] ДИАГНОСТИКА АВТО-ВЫЧИСЛЕНИЯ СТОИМОСТИ")
        print("=" * 60)

        # 1. Создаем два товара с разными ценами
        prod_url = f"/instances/{instance_uuid}/templates/{products_id}/notes"

        p1 = await test_client.post(
            prod_url,
            json={"data": {"name": "Браслет", "quantity_left": 10, "cost": 150}},
            headers=headers,
        )
        print(
            f"[DEBUG] Товар 1 создан | Status: {p1.status_code} | Response: {p1.json()}"
        )
        ...
        p2 = await test_client.post(
            prod_url,
            json={"data": {"name": "Цепочка", "quantity_left": 5, "cost": 350}},
            headers=headers,
        )
        print(
            f"[DEBUG] Товар 2 создан | Status: {p2.status_code} | Response: {p2.json()}"
        )
        assert p2.status_code == 201
        p2_uuid = p2.json()["_id"]

        # 2. Оформляем заказ (ПОЛЕ cost НАМЕРЕННО НЕ ПЕРЕДАЕМ)
        client_phone = "+375297777777"
        order_payload = {
            "data": {
                "product_list": [{"target_uuid": p1_uuid}, {"target_uuid": p2_uuid}],
                "client_phone": client_phone,
                "client_name": "Виктор Формульный",
                "pickup": True,
                "source": "сайт",
                "real_cost": 500,
            }
        ...
        print(f"[DEBUG] Отправляем Payload Заказа: {order_payload}")

        orders_url = f"/instances/{instance_uuid}/templates/{orders_id}/notes"
        order_resp = await test_client.post(
            orders_url, json=order_payload, headers=headers
        )

        print(f"[DEBUG] Ответ Создания Заказа | Status: {order_resp.status_code}")
        print(f"[DEBUG] Полный JSON Заказа: {order_resp.json()}")

        # Проверяем успешное создание
        assert order_resp.status_code == 201
        order_data = order_resp.json().get("data", {})

        print(f"[DEBUG] Извлеченный блок 'data' из ответа: {order_data}")
        print(f"[DEBUG] Фактическое значение 'cost' в ответе: {order_data.get('cost')}")
        print("=" * 60)

        # 3. ПРОВЕРЯЕМ ФИЧУ: Авто-расчет стоимости
        assert (
            "cost" in order_data
        ), "Критическая ошибка: Поле cost вообще отсутствует в ответе движка!"
        assert (
            order_data["cost"] == 500
        ), f"Ошибка: Ожидали 500, но движок вернул {order_data['cost']}"

        # 4. ПРОВЕРЯЕМ ТРИГГЕР: Создание клиента
        clients_url = f"/instances/{instance_uuid}/templates/{clients_id}/notes"
        c_list_resp = await test_client.get(clients_url, headers=headers)
        res_data = c_list_resp.json()

        print(f"[DEBUG] Состояние базы клиентов: {res_data}")
        print("=" * 60 + "\n")

        assert res_data["total"] == 1
        assert res_data["results"][0]["data"]["phone"] == client_phone
