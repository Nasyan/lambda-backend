...
class TestTriggerEngineV2BusinessCases:
    ...
    async def test_case_1_order_create_upserts_client(
        ...
        create_resp = await test_client.post(order_url, json=first_order, headers=headers)
        ...
        update_resp = await test_client.post(order_url, json=second_order, headers=headers)
    ...
    async def test_case_2_live_eval_returns_product_suggestions(
        ...
        await test_client.post(
            products_url,
            json={"data": {"name": "Кольцо распродано", "quantity_left": 0, "cost": 90}},
            headers=headers,
        )
        await test_client.post(
            products_url,
