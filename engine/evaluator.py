...
class ASTEvaluator:
    ...
    async def evaluate(self, node: ASTNode, scope: EvaluationScope) -> Any:
        ...
        if not handler:
            ...
            raise ValueError(
                f"Отсутствует обработчик для типа узла: {node.type}"
            )
    ...
    async def _eval_relation_field(
        ...
        relation_items = relation_value if isinstance(relation_value, list) else [
            relation_value
        ]
        relation_ids = [
    ...
    async def _eval_condition(
        self, node: ConditionNode, scope: EvaluationScope
    ) -> Any:
    ...
    async def _eval_object(self, node: ObjectNode, scope: EvaluationScope) -> Dict[str, Any]:
        return {
            field_name: await self.evaluate(field_node, scope)
            for field_name, field_node in node.fields.items()
        }

    async def _eval_query(self, node: QueryNode, scope: EvaluationScope) -> List[Dict[str, Any]]:
    ...
    async def _eval_binary_op(
        self, node: BinaryOpNode, scope: EvaluationScope
    ) -> Any:
        left_value = await self.evaluate(node.left, scope)
        right_value = await self.evaluate(node.right, scope)
        ...
        if node.operator in ("subtract", "multiply", "divide"):
            if not isinstance(left_value, (int, float)) or not isinstance(
                ...
                raise FormulaTypeMismatchError(
                    ...
                    right_val=right_value,
                    custom_message=(
                        f"Оператор '{node.operator}' "
                        "требует численных операндов."
                    ),
                )
